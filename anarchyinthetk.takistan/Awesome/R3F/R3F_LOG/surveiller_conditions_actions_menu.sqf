...
while {true} do {
	...
	if !(isNull _objet_pointe) then {
		if (player distance _objet_pointe < 13) then {
			...
			if ({_objet_pointe isKindOf _x} count R3F_LOG_CFG_objets_deplacables > 0) then {
				...
				R3F_LOG_action_deplacer_objet_valide = ((vehicle player == player) && ((count ([_objet_pointe] call vehicle_getCrew)) <= 0) && (isNull R3F_LOG_joueur_deplace_objet) &&
			...
			if ({_objet_pointe isKindOf _x} count R3F_LOG_CFG_objets_remorquables > 0) then {
				...
				if ({_objet_pointe isKindOf _x} count R3F_LOG_CFG_objets_deplacables > 0) then {
					...
					R3F_LOG_action_remorquer_deplace_valide = (vehicle player == player && (alive R3F_LOG_joueur_deplace_objet) &&
						((count ([_objet_pointe] call vehicle_getCrew)) <= 0) && (R3F_LOG_joueur_deplace_objet == _objet_pointe) &&
				...
				R3F_LOG_action_selectionner_objet_remorque_valide = (vehicle player == player && (alive _objet_pointe) && ((count ([_objet_pointe] call vehicle_getCrew)) <= 0) &&
			...
			if ({_objet_pointe isKindOf _x} count R3F_LOG_classes_objets_transportables > 0) then {
				...
				if ({_objet_pointe isKindOf _x} count R3F_LOG_CFG_objets_deplacables > 0) then {
					...
					R3F_LOG_action_charger_deplace_valide = (vehicle player == player && ((count([_objet_pointe] call vehicle_getCrew)) <= 0) && (R3F_LOG_joueur_deplace_objet == _objet_pointe) &&
				...
				R3F_LOG_action_selectionner_objet_charge_valide = (vehicle player == player && ((count([_objet_pointe] call vehicle_getCrew)) <= 0) &&
					isNull R3F_LOG_joueur_deplace_objet && isNull (_objet_pointe getVariable "R3F_LOG_est_transporte_par") &&
					(isNull (_objet_pointe getVariable "R3F_LOG_est_deplace_par") || (!alive (_objet_pointe getVariable "R3F_LOG_est_deplace_par"))) &&
					!(_objet_pointe getVariable "R3F_LOG_disabled"));
			};
	...
	if ({(vehicle player) isKindOf _x} count R3F_LOG_CFG_heliporteurs > 0) then {
		...
		R3F_LOG_action_heliporter_valide = (driver R3F_LOG_objet_addAction == player &&
			...
			isNull (R3F_LOG_objet_addAction getVariable "R3F_LOG_heliporte") && ([0,0,0] distance velocity R3F_LOG_objet_addAction < 6) && (getPosATL R3F_LOG_objet_addAction select 2 > 1) &&
		...
		R3F_LOG_action_heliport_larguer_valide = (driver R3F_LOG_objet_addAction == player && !isNull (R3F_LOG_objet_addAction getVariable "R3F_LOG_heliporte") &&
			([0,0,0] distance velocity R3F_LOG_objet_addAction < 15) && (getPosATL R3F_LOG_objet_addAction select 2 < 40) && !(R3F_LOG_objet_addAction getVariable "R3F_LOG_disabled"));
	};
