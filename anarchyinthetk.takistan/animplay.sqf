...
if ((_this select 0) == -1) exitWith {
	player groupChat localize "STRS_anim_noselect"
};
...
player setVariable ["animation_position", (getPosATL player)];

if (count((weapons player) - ["Binocular"] - ["NVGoggles"]) > 0) then {
	_hasweapon = true;
};

if ((not(_hasweapon)) and (_withWeapon == 1)) exitWith {
	player groupChat localize "STRS_anim_nur_mit_waffe";
};

if ((_hasweapon) and (_withWeapon == 0)) exitWith {
	player groupChat localize "STRS_anim_nur_ohne_waffe";
};

if (_callScript != "") exitWith {
	[_animnumber, _animScriptName] execVM _callScript;
};
